...
def create_phoneme_list(accent_phrases: List[AccentPhrase]):
    ...
    class Ap:
        def __init__(
                self, context: Context, bg: Bg, accent: Optional[int] = None,
                 is_interrogative: Optional[bool] = False, has_pau: Optional[bool] = False
            ) -> None:
    ...
    class ContextLabel:
        def __init__(self,context:Context, ap:Optional[Ap], mo:Optional[Mo], phoneme: str) -> None:
        ...
        def g5(self):
            if self._phoneme == 'sil' or self._phoneme == 'pau':
                return 'xx'
            if self._ap == None or len(self._context.accent_phrases) <= self._context.accent_phrases.index(self._ap) + 2:
                return 'xx'
        ...
        def label(self) -> Phoneme:
            context = {
                ...
                'c1':'xx', 'c2':'xx', 'c3':'xx',
                'd1':'xx', 'd2':'xx', 'd3':'xx',
                'e1':self.e1(), 'e2':self.e2(), 'e3':self.e3(), 'e4':self.e4(), 'e5':self.e5(),
                'f1':self.f1(), 'f2':self.f2(), 'f3':self.f3(), 'f4':self.f4(), 'f5':self.f5(), 'f6':self.f6(), 'f7':self.f7(), 'f8':self.f8(),
                'g1':self.g1(), 'g2':self.g2(), 'g3':self.g3(), 'g4':self.g4(), 'g5':self.g5(),
                'h1':self.h1(), 'h2':self.h2(),
                'i1':self.i1(), 'i2':self.i2(), 'i3':self.i3(), 'i4':self.i4(), 'i5':self.i5(), 'i6':self.i6(), 'i7':self.i7(), 'i8':self.i8(),
    ...
    for ii, i in enumerate(sprit_index[1:]):
        ...
        for accent_phrase in b_group:
            ap = Ap(context, bg, accent_phrase.accent, accent_phrase.is_interrogative,bool(accent_phrase.pause_mora))
...
class MockSynthesisEngine(SynthesisEngineBase):
    ...
    def _synthesis_impl(self, query: AudioQuery, speaker_id: int) -> np.ndarray:
        ...
        wave, sr = pyopenjtalk.synthesize([i.label().label for i in create_phoneme_list(query.accent_phrases)])
